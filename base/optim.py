...
def sgd(
		...
		nesterov: bool = False,
		**kwargs, ):
	def sgd_transform(grad):
		new_state = {}
		if state["t"] > 0:
			new_state["momentum_buffer"] = (
				state["momentum_buffer"] * momentum
				+ (1 - dampening) * grad
			)
		else:
			new_state["momentum_buffer"] = grad
		if nesterov:
			grad = grad + momentum * new_state["momentum_buffer"]
		else:
			grad = new_state["momentum_buffer"]
		update = -lr * grad
